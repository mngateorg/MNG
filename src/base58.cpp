...
namespace {
    class CMNGateAddressVisitor : public boost::static_visitor<bool> {
    private:
        CMNGateAddress *addr;
    public:
        CMNGateAddressVisitor(CMNGateAddress *addrIn) : addr(addrIn) { }
...
bool CMNGateAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CMNGateAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CMNGateAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CMNGateAddressVisitor(this), dest);
}

bool CMNGateAddress::IsValid() const {
...
CTxDestination CMNGateAddress::Get() const {
    if (!IsValid())
...
bool CMNGateAddress::GetKeyID(CKeyID &keyID) const {
...
bool CMNGateAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CMNGateSecret::SetKey(const CKey& vchSecret) {
...
CKey CMNGateSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CMNGateSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CMNGateSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CMNGateSecret::SetString(const std::string& strSecret) {
