...
int nAnonymizeMNGateAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MNGate
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MNGate
    // Mac: ~/Library/Application Support/MNGate
    // Unix: ~/.MNGate
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNGate";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MNGate.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MNGated.pid"));
