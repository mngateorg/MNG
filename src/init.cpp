...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MNGate.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MNGated.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMNGateamount=<n> " + _("Keep N MNGate anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MNGate is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MNGate is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MNGate version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MNGate server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MNGate") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MNGate to complete") << "\n";
    ...
    nAnonymizeMNGateAmount = GetArg("-anonymizeMNGateamount", 0);
    if(nAnonymizeMNGateAmount > 999999) nAnonymizeMNGateAmount = 999999;
    if(nAnonymizeMNGateAmount < 2) nAnonymizeMNGateAmount = 2;
    ...
    LogPrintf("Anonymize MNGate Amount %d\n", nAnonymizeMNGateAmount);
