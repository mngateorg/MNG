...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMNGateAmount"))
        settings.setValue("nAnonymizeMNGateAmount", 1000);
    nAnonymizeMNGateAmount = settings.value("nAnonymizeMNGateAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMNGateAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMNGateAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMNGateAmount:
            return QVariant(nAnonymizeMNGateAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMNGateAmount:
            nAnonymizeMNGateAmount = value.toInt();
            settings.setValue("nAnonymizeMNGateAmount", nAnonymizeMNGateAmount);
            emit AnonymizeMNGateAmountChanged(nAnonymizeMNGateAmount);
            break;
        default:
            break;
        }
