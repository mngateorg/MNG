...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMNGateAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMNGateAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMNGateAmount = strAnonymizeMNGateAmount.remove(strAnonymizeMNGateAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMNGateAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMNGateAmount*COIN) nMaxToAnonymize = nAnonymizeMNGateAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMNGateAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMNGateAmount));
        strAnonymizeMNGateAmount = strAnonymizeMNGateAmount.remove(strAnonymizeMNGateAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMNGateAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
